...
class _AnimationStudioScreenState extends State<AnimationStudioScreen> {
  ...
  List<Widget> _jiggleControls(AppState app) {
    return <Widget>[
      ...
      const SizedBox(height: 4),
      const Text('Motion styles (more ways to move)',
          style: TextStyle(fontSize: 12, color: Colors.white70)),
      _jiggleSlider('Circular (2D ellipse)', _jiggle.crossAmount, 0, 1,
          (double v) => _jiggle = _jiggle.copyWith(crossAmount: v),
          hint: 'The tip traces a little ellipse instead of straight up/down'),
      _jiggleSlider('Swirl (rotation)', _jiggle.swirl, 0, 20,
          (double v) => _jiggle = _jiggle.copyWith(swirl: v),
          suffix: '°', divisions: 40, hint: 'A back-and-forth rotation wobble'),
      _jiggleSlider('Pulse (swell & fade)', _jiggle.pulse, 0, 1,
          (double v) => _jiggle = _jiggle.copyWith(pulse: v),
          hint: 'The jiggle breathes — strength rises and falls each loop'),
